...
Коментарий к диаграмме:
- На диаграмме система (Онлайн кинотеатр "Кинобездна) это и есть домен компании.
- Микросервисы, реализующие контексты, сгруппированы по поддоменам.
- Для каждого микросервиса обозначено, кикой контекст (или контексты) он реализует.
- Если микросервис использует БД, то для него и контейнера СУБД с этой БД обозначена граница.
